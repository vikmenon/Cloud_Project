...
public class ThumbsGenerator {
	...
	public static void main(String[] args) {
    	File inputFile = new File("MyVideo.mp4");
    	generateThumbnailsForFile(inputFile, ThumbsGenerator.AllFramesDir);
		System.exit(0);
    }
	
	public static void generateThumbnailsForFile(File localFile, String outputFramesDirArg) {
		// Append the separator for building image file (frame) paths
		String outputFramesDir = outputFramesDirArg + '/';
		System.out.println("oFD is: " + outputFramesDir);
		System.out.println("Reading file: " + localFile.getAbsolutePath());
		currentVideoName = localFile.getName();
		if(!createImageDirectory(outputFramesDir)){
			System.out.println("WARN: Did not create the directory (may already exist): " + outputFramesDirArg);
		}
		try {
			captureVideoFrames(localFile.getAbsolutePath(), outputFramesDir);
	...
	private static String dumpImageToFile(BufferedImage image, String outputFramesDir) {
        try {
        	 String outputFilename = outputFramesDir + currentVideoName + '/' + System.currentTimeMillis() + ".jpg";
