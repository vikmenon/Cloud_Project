...
public class SimilarFrameEliminator {
	...
	public static final String ReducedFramesDir = "FinalFrames";

	public SimilarFrameEliminator(ArrayList<File> allFiles) throws IOException {
	...
	public static void eliminateFrames(String inputPathArg, String outputPathArg) {
		String inputDir = inputPathArg + File.separator;
		String outputDir = outputPathArg + File.separator;
		File dir = new File(inputDir);
		
		ArrayList<File> allFiles = new ArrayList<File>(Arrays.asList(dir.listFiles(new JPEGImageFileFilter())));
		...
		for (final File fileEntry : dir.listFiles()) {
    		...
    		allFiles = new ArrayList<File>(Arrays.asList(dir.listFiles(new JPEGImageFileFilter())));
    		
    		System.out.println("Total Initial frame count "+allFiles.size());
    		
    		try {
	    		new SimilarFrameEliminator(allFiles);
	    		System.out.println("Final frame count "+allFiles.size());
	    		createImageDirectory(outputDir,videoName);
	    		for(int i=0;i<allFiles.size();i++){
	    			Files.copy(allFiles.get(i).toPath(), new File(outputDir + videoName + File.separator + allFiles.get(i).getName()).toPath());
	    		}
    		}
    		catch (IOException e) {
    			System.out.println("ERR: IOException while attempting to eliminate similar frames!");
    			e.printStackTrace();
    		}
		}
	}
