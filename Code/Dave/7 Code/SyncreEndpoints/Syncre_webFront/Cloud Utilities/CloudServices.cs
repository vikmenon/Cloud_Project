...
namespace CloudServices
{
    ...
    public static class CloudServices
    {
        ...
        //upload specifeied video to cloud store  for processing
        public static void UploadVideo(string videofilenameAndPath, string destFilename, ref string statusMsg)
        {
            aws_s3.Write(videofilenameAndPath, videoLocalFolder, destFilename, ref statusMsg);

            if (statusMsg != "")
            {
                LogManager.Write(LogManager.LogType.ERROR, statusMsg);
                statusMsg = "Error while downloading video from cloud services.";
            }
            else
                statusMsg = "";
        }
        ...
        public static void RemoveObjects(List<string> objectHandles, ref string statusMsg)
        {
